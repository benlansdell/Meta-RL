...
class confounded_env(object):
    def __init__(self, delay = 1, p1 = 0.2, p2 = 0.01, p3 = 0.01, max_steps = 100, obs_steps= 90, chain_prob = 0.5):
        self.delay = delay
        self.N = 3
        self.p1 = p1
        self.p2 = p2
    ...
    def reset(self):
        ...
        self.is_chain = uniform() > self.chain_prob
    ...
    def step(self,action):
        ...
        x2 = y2 + (1-y2)*xhistory[max(0, self.timestep - 1), 0]
        #Depending on topology, choose if node C is active
        if self.is_chain:
        	x3 = y3 + (1-y3)*xhistory[max(0, self.timestep - 1), 1]
	    else:
    	    x3 = y3 + (1-y3)*xhistory[max(0, self.timestep - 2), 0]
        ...
        if self.timestep >= self.obs_steps:
        	reward = 2*(action == self.is_chain) - 1
        else:
        	reward = 0
...
class AC_Network():
    def __init__(self,a_size,scope,trainer):
        with tf.variable_scope(scope):
            ...
            self.curr_states = tf.placeholder(shape = [None, 3], dtype = tf.int32)
            self.curr_states_onehot = tf.one_hot(self.curr_states, 3, dtype=tf.float32)

            self.timestep = tf.placeholder(shape=[None,1],dtype=tf.float32)
            self.prev_actions_onehot = tf.one_hot(self.prev_actions,a_size,dtype=tf.float32)

            hidden = tf.concat([self.prev_rewards,self.prev_actions_onehot,self.timestep, self.curr_states_onehot],1)
        ...
        with sess.as_default(), sess.graph.as_default():                 
            while not coord.should_stop():
                ...
                while d == False:
                    ...
                    episode_buffer.append([a,r,t,d,v[0,0],s])
...
with tf.device("/cpu:0"): 
    ...
    for i in range(num_workers):
        workers.append(Worker(confounded_env(), i, a_size, trainer, model_path, global_episodes))
